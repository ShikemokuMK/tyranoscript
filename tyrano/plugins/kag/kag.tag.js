...
<br>`left`、`left_center`、`center`、`right_center`、`right`の代わりに、それぞれ`l`、`lc`、`c`、`rc`、`r`を指定することもできます。（動作は同じです）。
<br>この属性を指定した場合は`left`属性や`top`属性は無視されます。
<br>`layer`を`base`と指定した場合、この属性は指定しないでください。各々の表示位置は`Config.tjs`で事前に設定しておきましょう。
...
:exp
レイヤにテキストを表示します。前景レイヤに対してのみ実行します。

文字を消す時は`[freeimage]`タグを使うか、このタグで指定しておいた`name`属性に対して`[free]`タグを使う必要があります。
