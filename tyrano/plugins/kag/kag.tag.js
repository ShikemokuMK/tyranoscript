...
tyrano.plugin.kag.ftag ={
    ...
    //次の命令を実行する //ロードした時の状態を再現するためには。。。レイヤ以下をHTMLとして保存していいじゃん。。。もう。それを構築すればいいよ。で、
    nextOrder:function(){
...
レイヤ画像の解放
:exp
imageタグでレイヤに追加した画像を解放（非表示）にします。レイヤ指定は必須です。
:sample
[backlay]
;キャラクター表示
...
target=ャンプ先のラベルを指定します。省略すると、ファイルの先頭から実行されます。
#[end]
*/
...
fix=true falseで指定します。デフォルトはfalse 。trueを指定すると、Fixレイヤーにボタンが配置されます。この場合、ボタンを表示してもシナリオを進める事ができます。例えば、セーブボタンといった常に表示したいボタンを配置する時に活用できます。また、fixレイヤーに追加した要素を消す場合はfixclearタグ を使います,
