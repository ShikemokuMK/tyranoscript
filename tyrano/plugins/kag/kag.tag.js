...
frame        = メッセージレイヤのフレーム画像として表示させる画像を指定します。<br>メッセージエリアをカスタマイズしたい場合に利用できます。<br>画像サイズは`width`と`height`属性に準じて調整して下さい。<br>さらに、`margin`属性で実際にメッセージが表示される箇所の調整も行いましょう。<br>また、`none`と指定することで標準枠に戻すこともできます。違う枠画像をしていすると切り替えることもできます,
...
:exp
指定したレイヤ（あるいはすべてのレイヤ）の情報（レイヤに含まれる画像などの要素）を、表ページから裏ページにコピーします。
...
face         = フォントの種類を指定します。Webフォントを使用する場合は`tyrano/css/font.css`に定義を記述して下さい。,
...
一時的な演出で文字速度を変更したい場合はこのタグではなく`[delay]`タグを使用します。
...
height     = ボタンの高さをピクセルで指定できます。,
size       = フォントサイズを指定できます。,
face       = フォントを指定できます。Webフォントを使用する場合は`tyrano/css/font.css`に定義を記述して下さい。,
graphic    = ボタンの背景画像を指定します。ファイルは`data/image`フォルダに入れて下さい。画像が指定された場合は`color`は無視されます。,
...
x            = 領域の左端位置のX座標を指定します。,
y            = 領域の左端位置のY座標を指定します。,
borderstyle  = 領域に表示する線のデザインを指定できます。`線の太さ:線の種類:線の色`のフォーマットで記述して下さい。各項目は<a href="https://developer.mozilla.org/ja/docs/Web/CSS/border" target="_blank">CSSの記法</a>で記述します。線の種類は`solid``double``groove``dashed dotted`などが指定できます。,　
